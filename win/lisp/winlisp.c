...
struct window_procs lisp_procs = {
    ...
    (WC2_DARKGRAY | WC2_HITPOINTBAR
     ...
     | WC2_PETATTR | WC2_GUICOLOR | WC2_SUPPRESS_HIST | WC2_URGENT_MESG
     | WC2_MENU_SHIFT),
                ...
                for (i = 0; races[i].noun; i++)
                    if (ok_race(flags.initrole, i, flags.initgend,
                        ...
                        add_menu(win, &nul_glyphinfo, &any, races[i].noun[0],
                                 0, ATR_NONE, NO_COLOR, races[i].noun,
                                 MENU_ITEMFLAGS_NONE);
                    }
                ...
                for (i = 0; i < ROLE_GENDERS; i++)
                    if (ok_gend(flags.initrole, flags.initrace, i,
                        ...
                        add_menu(win, &nul_glyphinfo, &any, genders[i].adj[0],
                                 0, ATR_NONE, NO_COLOR, genders[i].adj,
                                 MENU_ITEMFLAGS_NONE);
                    }
                ...
                for (i = 0; i < ROLE_ALIGNS; i++)
                    if (ok_align(flags.initrole, flags.initrace,
                        ...
                        add_menu(win, &nul_glyphinfo, &any, aligns[i].adj[0],
                                 0, ATR_NONE, NO_COLOR, aligns[i].adj,
                                 MENU_ITEMFLAGS_NONE);
                    }
...
get_options(void)
{
    lisp_cmd(
        ...
        // opts in config file *\/ */ lisp_int(*iflags.opt_compdup);   /\* for
        // duplication of compound opts in config file *\/ */
...
void
lisp_status_update(int idx, genericptr_t ptr, int chg UNUSED, int percent,
...
void
lisp_add_menu(winid window, /* window to use, must be of type NHW_MENU */
...
void
lisp_end_menu(winid window,       /* menu to use */
...
lisp_init_nhwindows(int *argcp, char **argv)
{
    ...
    if (read_string("string", &need_options_file_p) != -1) {
        if (!strcmp(need_options_file_p, "t")) {
            char buf[BUFSZ];
            FILE *fp = fopen(get_configfile(), "r");

            if (fp == NULL) {
                Sprintf(buf, "cannot open options file %s", get_configfile());
...
void
lisp_print_glyph(winid window, coordxy x, coordxy y,
                 ...
                 const glyph_info *bkglyphinfo UNUSED)
{
    ...
    if (window == WIN_MAP) {
        lisp_cmd("print-glyph",
                 lisp_int(x);
                 lisp_int(y); lisp_int(color); lisp_int(glyph);
...
win_request_info *
lisp_ctrl_nhwindow(winid window UNUSED, int request UNUSED,
